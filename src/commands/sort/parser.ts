...
/**
 * Parse multiple key specs from command line arguments
 */
export function parseKeyArgs(args: string[]): {
  keys: KeySpec[];
  ...
  while (i < args.length) {
    ...
    if (arg === "-k" || arg === "--key") {
      if (i + 1 < args.length) {
        const keySpec = parseKeySpec(args[i + 1]);
        if (keySpec) {
          keys.push(keySpec);
        }
        i += 2;
